...
int	pb(t_root *stack_a, t_root *stack_b)
{
	...
	first_node_a->next->prev = first_node_a->prev;
	first_node_a->prev->next = first_node_a->next;
	new_node = ut_create_node(stack_a, tmp);
	if (new_node == NULL)
		return (-1);
...
void	ra(t_root *stack_a)
{
	...
	first_node = (stack_a->sentinel)->next;
	first_node = first_node->next;// これでfirstノードの指すポインタは変更されるのか？
}
